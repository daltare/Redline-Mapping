...
# Get list of CES 3 variable names and plain text names
    ces_variables <- read_csv(here('data_prepared', 
                                    'ces_names.csv'))
...
```{r load data}
    # load data
        # Redline Polygons
            redline_polygons <- read_rds(here('data_prepared', 'redline_polygons.gpkg'))
    ...
    # loop through all HOLC polygons ...
        ...
        # CES 3 Polygons
            ces_poly <- read_sf(here('data_raw', 
                                     'CalEnviroScreen3', 
                                     'CESJune2018Update_SHP',
                                     'ces3June2018Update.shp'))
                # st_crs(ces_poly) # to check the reference system
            # modify the 'City' column to better keep track when joining with other datasets
                ces_poly <- ces_poly %>% rename('CES_City' = 'City')
                
        # Transform both to UTM Zone 10N
            redline_polygons <- redline_polygons %>% st_transform(crs = 26910)
            ces_poly <- ces_poly %>% st_transform(crs = 26910)
            
    # create a map to check that the data loaded correctly
        # tm_shape(ces_poly) + tm_borders() + tm_shape(redline_polygons) + tm_borders(col = 'red')
...
```{r basemap-background}
    # get a basemap (See: https://github.com/mtennekes/tmap/issues/185)
        ...
        background <- cc_location(ces_poly %>% 
                                      filter(CES_City == 'Sacramento'), 
                                  verbose = FALSE)
```
...
```{r ces-redline intersection}
    # Fix self-intersecting polygons
        if (sum(!st_is_valid(ces_poly)) > 0) {
            ces_poly <- sf::st_buffer(ces_poly, dist = 0)
        }
    ...
    # get the intersection of the CES polygons and redline polygons
        ces_redline_intersection <- st_intersection(ces_poly, redline_polygons)
    # inspect
...
```{r ces polygon map Function}
    ces_poly_map <- function(ces_id, ces_measure_name) {
        ...
        map_ces <- tm_shape(background) + 
            tm_rgb(alpha = 0.5) + 
            tm_shape(ces_poly %>% filter(CES_City == 'Sacramento' | CES_City == 'West Sacramento')) + 
            tm_polygons(col = ces_id, title = ces_measure_name, border.alpha = 1) + 
            tm_shape(ces_redline_intersection %>% filter(redline_city == 'Sacramento'), is.master = TRUE) +
...
```{r CES-Redline Overlap Map Function}
    ces_redline_overlap_map <- function(ces_id, ces_measure_name) {
        map_overlap <- tm_shape(background) + 
            tm_rgb(alpha = 0.5) + 
            tm_shape(ces_poly %>% filter(CES_City == 'Sacramento')) + 
            tm_borders(lwd = 1) + 
            tm_shape(ces_redline_intersection %>% filter(redline_city == 'Sacramento'), is.master = TRUE) + 
...
```{r Facet Map Function}
    ...
    ces_redline_facet_map <- function(ces_id, ces_measure_name) {
        map_facets <- tm_shape(ces_poly %>% filter(CES_City == 'Sacramento')) +
            tm_borders(lwd = 1) +
            tm_shape(background) +
            tm_rgb(alpha = 0.5) +
            tm_shape(ces_redline_intersection %>% 
                         filter(redline_city == 'Sacramento'), is.master = TRUE) +
            # tm_shape(ces_redline_intersection %>% filter(redline_city == 'Sacramento')) +
...
```{r modified bar plot function - city plus statewide}
ces_scores_plot <- function(ces_id, ces_measure_name) {
    # TROUBLESHOOTING
        ...
        ces_id <- ces_measures_effects_scores$id[i_map]
    ...
    # compute weighted average score for each HOLC ID (first compute for ces_redline_intersection polygon
        holc_poly_wt_score <- ces_redline_intersection %>% 
            ...
            group_by(redline_city, holc_id_2, holc_grade) %>% # compute weighted avg score for each HOLC polygon (combining individual ces_redline_intersection polygons that are within each HOLC polygon)
            summarize(total_area = sum(clipped_area), 
                      area_x_score_total = sum(area_x_score), 
    ...
    # box plot
        departures_boxplot <- ggplot(holc_poly_wt_score_departures, 
                                     aes(x = holc_grade, 
    ...
    # point plot
        departures_point <- ggplot(holc_poly_wt_score_departures) + 
            geom_point(aes(x = !!as.name(paste0(var_name, '_departure')), y = redline_city, color = holc_grade))
        
    # point plot - by city
        departures_point_city <- ggplot(holc_poly_wt_score_departures %>% 
                                            filter(redline_city == 'Sacramento')) + 
            geom_point(aes(x = !!as.name(paste0(var_name, '_departure')), y = holc_grade, color = holc_grade))
...
# compute average departure for each HOLC grade within each city
    ...
    holc_city_means_plot <- ggplot(holc_poly_wt_score_departures_summary) +
        geom_point(aes(x = city_holc_dep_average, y = redline_city, color = holc_grade))
    
    holc_city_medians_plot <- ggplot(holc_poly_wt_score_departures_summary) +
        geom_point(aes(x = city_holc_dep_median, y = redline_city, color = holc_grade))
